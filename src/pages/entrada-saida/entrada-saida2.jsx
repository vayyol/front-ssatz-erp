...
function registros() {
    ...
    async function LoadMovimentacoesEstoque() {
        try {
            ...
            const movimentacoesEstoque = resposta.data.map((m) => ({
                id: m.id,
                quantidade: m.quantidade,
            ...
            setMovimentacoesEstoque(movimentacoesEstoque);
        } catch (err) {
            console.error(err);
    ...
    //cria uma constante de registros 
    const registrosEstoque = movimentacoesEstoque.map((mov) => ({
        ...mov,
    ...
    // Junta a lista registros com a lista registrosEstoque
        const registrosGeral = [
        ...
        ...registrosEstoque.map((registro) => ({

            ...registro,

            idUnico: `estoque-${registro.id}`,
    ...
    console.log("REGISTROS GERAL:", registrosGeral);

   [
    {
        id: 106,
        idUnico: "registro-106",
        ...
        vencimento: "2026-10-10T00:00:00",
        vencimentoFormatado: "10/10/2026",

        created_at: "2026-08-09T03:04:52.848344",
        created_atFormatado: "09/08/2026",
    },

    {
        ...
        created_at: "2026-08-09T00:05:26.516106",
        created_atFormatado: "09/08/2026",
    }
]
    ...
    const tiposCustosEstoque = [
        ...new Set(movimentacoes.map((registro) => registro.tipo))
    ];

    // ABAIXO SÃO OS FILTROS DE BUSCA DA BARRA DE PESQUISA E DO MODAL
    const registrosFiltrados = registrosGeral.filter((registro) => {
        ...
        if (
            valorMin !== "" &&
            Number(registro.valorTotal ?? 0) < Number(valorMin)
        ) {
            return false;
        }
        ...
        if (
            valorMax !== "" &&
            Number(registro.valorTotal ?? 0) > Number(valorMax)
        ) {
            return false;
        }
        ...
        if (
            dataInicial &&
            new Date(registro.created_at) < new Date(dataInicial)
        ) {
            return false;
        }
        ...
        if (
            dataFinal &&
            new Date(registro.created_at) > new Date(dataFinal)
        ) {
            return false;
        }
                                ...
                                {hoverRegistro.origem === "estoque" ? (
                                    ...
                                    <div className="mt-2 space-y-1">
                                        ...
                                        <p>
                                            ...
                                            {hoverRegistro.estoqueAnterior ?? 0}
                                        </p>

                                        <p>
                                            <strong>Estoque depois:</strong>{" "}
                                            {hoverRegistro.estoqueDepois ?? 0}
                                        </p>
        ...
        >
            ...
            ======================== */}
                            ...
                            <select
                                ...
                                onChange={(e) =>
                                    setFiltroMovimento(e.target.value)
                                }
                                style={{
                                    width: "100%",
                                    padding: "10px",
